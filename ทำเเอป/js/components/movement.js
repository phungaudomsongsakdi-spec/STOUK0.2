const MovementComponent = {
        ...
        <div class="card-body">
          <div style="background:#f8fafc; border-radius: 24px; padding: 20px; margin-bottom: 24px;">
            ...
            <div class="form-row" style="margin-top: 12px;">
              ...
              <div class="form-group"><label>ชื่อผู้รับ/หมายเหตุ</label><input type="text" id="movementNote" placeholder="ชื่อพนักงาน / งานที่ใช้"></div>
            ...
            <table style="min-width:1100px; border-collapse: collapse; width:100%;">
              <thead>
                <tr style="border: 1px solid #ddd;">
                  ...
                  <th style="width:15%; text-align:center; border:1px solid #ddd; padding:10px;">ประเภท</th>
                  <th style="width:10%; text-align:center; border:1px solid #ddd; padding:10px;">Itemcode</th>
                  ...
                  <th style="width:8%; text-align:center; border:1px solid #ddd; padding:10px;">ราคา/หน่วย</th>
                  <th style="width:8%; text-align:center; border:1px solid #ddd; padding:10px;">มูลค่า</th>
                  <th style="width:12%; text-align:center; border:1px solid #ddd; padding:10px;">ผู้จ่าย</th>
                  <th style="width:7%; text-align:center; border:1px solid #ddd; padding:10px;">แผนก</th>
                  <th style="width:12%; text-align:center; border:1px solid #ddd; padding:10px;">หมายเหตุ/ผู้รับ</th>
      ...
      html += `
        <div class="dropdown-item" data-code="${p.itemcode}" data-name="${p.description}" data-price="${p.price}" data-stock="${p.quantity}" 
          ...
          <div style="text-align:right;">
            <span style="font-weight:600; ${stockColor}">${p.quantity}</span> ชิ้น<br>
  ...
  recordMovement() {
    ...
    alert(`บันทึกสำเร็จ: ${typeLabels[type]} ${qty} ชิ้น มูลค่า ${totalVal.toFixed(2)} บาท (แผนก: ${this.selectedDepartment})`);
  },
  ...
  renderMovementsHistory(filterText = "", typeFilter = "", deptFilter = "", fromDate = "", toDate = "") {
    ...
    if(!tbody) return;
    if(filtered.length === 0) { 
      tbody.innerHTML = "<td><td colspan='11' style='text-align:center; padding:40px;'>ไม่มีประวัติ</td</tr>";
      return; 
    }
    
    let typeClass = {
      anniversary: "type-event",
      probation: "type-staff",
      general: "type-staff",
      newhire: "type-staff"
    };
    ...
    filtered.forEach(m => {
      ...
      let badgeClass = typeClass[m.type] || "type-staff";
      ...
      html += `
        <tr style="border: 1px solid #ddd;">
          ...
          <td style="text-align:center; border:1px solid #ddd; padding:10px;"><span class="type-badge ${badgeClass}">${typeText}</span></td>
          <td style="text-align:center; border:1px solid #ddd; padding:10px;">${m.itemcode}</td>
          <td style="text-align:left; border:1px solid #ddd; padding:10px;">${m.description}</td>
          <td style="text-align:center; border:1px solid #ddd; padding:10px;">${m.quantity}</td>
          <td style="text-align:right; border:1px solid #ddd; padding:10px;">${(m.unitPrice || 0).toFixed(2)}</td>
          <td style="text-align:right; border:1px solid #ddd; padding:10px;">${(m.totalValue || 0).toFixed(2)}</td>
          <td style="text-align:center; border:1px solid #ddd; padding:10px;">${m.issuer || "-"}</td>
          <td style="text-align:center; border:1px solid #ddd; padding:10px;">${m.department || "-"}</td>
          <td style="text-align:center; border:1px solid #ddd; padding:10px;">${m.note || '-'}</td>
  ...
  exportToExcel() {
    try {
      ...
      // สร้าง HTML สำหรับ Excel (Excel สามารถเปิดไฟล์ HTML ได้)
      let htmlContent = `
        <html>
        <head>
          <meta charset="UTF-8">
        ...
        <body>
          ...
          <div class="report-date">
            ...
            ${typeFilter ? ` | ประเภท: ${typeLabels[typeFilter] || typeFilter}` : ''}
            ${deptFilter ? ` | แผนก: ${deptFilter}` : ''}
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style="width:15%">หมายเหตุ/ผู้รับ</th>
      ...
      filtered.forEach(m => {
        ...
        htmlContent += `
              <tr>
                ...
                <td style="text-align:center">${m.itemcode}</td>
                ...
                <td style="text-align:right">${(m.totalValue || 0).toFixed(2)}</td>
                <td style="text-align:left">${m.issuer || "-"}</td>
                <td style="text-align:center">${m.department || "-"}</td>
      ...
      // สร้าง Blob เป็นไฟล์ .xls (Excel สามารถเปิดได้)
